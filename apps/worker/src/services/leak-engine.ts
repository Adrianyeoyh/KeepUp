import type { LeakType, MetricName } from '@flowguard/shared';
import { query } from '../db/client.js';
import { logger } from '../logger.js';
import { evaluateCustomLeakRules } from './jql-custom-leak-rules.js';
...
async function getLatestMetric(companyId: string, metricName: MetricName, teamId?: string): Promise<LatestMetric | null> {
...
async function leakExistsForToday(companyId: string, leakType: LeakType, teamId?: string): Promise<boolean> {
...
async function getSlackDecisionEvidence(companyId: string, teamId?: string): Promise<EvidenceLink[]> {
  const teamClause = teamId ? `AND team_id = $2` : '';
  const result = await query<{ channel_id: string | null; thread_ts: string | null; message_count: string }>(
    `SELECT
...
async function getSlackUnloggedActionEvidence(companyId: string): Promise<{ count: number; evidence: EvidenceLink[] }> {
  const result = await query<{ channel_id: string | null; thread_ts: string | null; implied_count: string }>(
    `SELECT
      metadata->>'channel_id' AS channel_id,
      metadata->>'thread_ts' AS thread_ts,
...
async function getJiraEvidence(companyId: string, eventType: 'jira.issue_reopened' | 'jira.issue_transitioned'): Promise<EvidenceLink[]> {
...
export async function runLeakDetection(companyId: string): Promise<void> {
  // 1. Company-level leak detection (existing behavior)
  await detectLeaks(companyId);
  ...
  for (const team of teamsResult.rows) {
    try {
      await detectLeaks(companyId, team.id);
    } catch (err) {
      logger.warn({ err, companyId, teamId: team.id }, 'Team leak detection failed — continuing');
    }
  }

  // 3. Custom JQL-powered leak rules (per-team)
  try {
    await evaluateCustomLeakRules(companyId);
  } catch (err) {
    logger.warn({ err, companyId }, 'Custom JQL leak rule evaluation failed — continuing');
  }
}

async function detectLeaks(companyId: string, teamId?: string): Promise<void> {
