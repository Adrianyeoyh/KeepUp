 ...
 *
 * Generates a per-team retrospective from metadata every sprint (biweekly).
...
export async function runSprintRetrospective(companyId: string): Promise<void> {
  ...
  if (!slackToken) {
    log.debug('No Slack token — skipping sprint retro');
    return;
  }
  ...
  for (const team of teamsResult.rows) {
    try {
      ...
      // Post to Slack
      if (retro.slackChannel) {
        await postRetroToSlack(slack, retro);
...
async function generateTeamRetro(
  ...
  // Parallel queries for all retro data
  ...
  // Parse metrics into structured form
  ...
  return {
    ...
    metrics: {
      ...
      leakCount: {
        ...
        previous: 0, // simplified — could query previous sprint
      },
    },
...
async function postRetroToSlack(slack: WebClient, retro: RetroData): Promise<void> {
  if (!retro.slackChannel) return;
  ...
  if (retro.metrics.cycleTime && retro.metrics.cycleTime.current < retro.metrics.cycleTime.previous) {
    ...
    wellItems.push(`⏱️ Cycle time improved by *${pct}%*`);
  }
  ...
  if (retro.metrics.cycleTime && retro.metrics.cycleTime.current > retro.metrics.cycleTime.previous * 1.1) {
    ...
    badItems.push(`⏱️ Cycle time increased by *${pct}%*`);
  }

  if (badItems.length > 0) {
    blocks.push({
      type: 'section',
      text: { type: 'mrkdwn', text: `*⚠️ What didn't go well*\n${badItems.join('\n')}` },
    });
  }
