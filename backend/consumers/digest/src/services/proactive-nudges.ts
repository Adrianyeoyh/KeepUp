 ...
 * Migrated from apps/worker. Uses adapterRegistry for Slack delivery.
 * 5 nudge types: reviewer bottleneck, epic risk, shadow work,
 * cross-team collision, meeting-to-action ratio.
 */
...
export async function runProactiveNudges(companyId: string): Promise<void> {
  ...
  if (!adapterRegistry.has('slack')) {
    log.debug('Slack adapter not registered — skipping nudges');
    return;
  }
  ...
  for (const nudge of nudges) {
    try {
      await adapterRegistry.executeAction({
        provider: 'slack',
        actionType: nudge.isDM ? 'send_dm' : 'post_message',
        targetId: nudge.channel,
        companyId,
        payload: {
          text: nudge.text,
          blocks: nudge.blocks,
        ...
        riskLevel: 'low',
        metadata: { nudge_type: nudge.type },
      });
...
async function detectReviewerBottlenecks(companyId: string): Promise<NudgeCandidate[]> {
  ...
  for (const row of result.rows) {
    ...
    nudges.push({
      ...
      channel: target,
      isDM: true,
      text: `Reviewer bottleneck: ${row.reviewer} has ${row.pending_count} pending reviews`,
      blocks: [
        {
          type: 'section',
          text: {
        ...
        {
          type: 'context',
          elements: [{ type: 'mrkdwn', text: '_FlowGuard Proactive Nudge_ · <https://flowguard.dev|View dashboard>' }],
        },
      ],
    });
  }
