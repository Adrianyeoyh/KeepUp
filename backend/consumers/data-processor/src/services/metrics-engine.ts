...
/**
 * Metrics Engine — Aggregates daily metric snapshots from raw events.
 *
 * Migrated from apps/worker/src/services/metrics-engine.ts.
 * Key changes:
...
async function getBaseline(
  ...
  if (!result.rows[0]?.baseline) return null;
...
async function upsertSnapshot(
  ...
  await query(
    `INSERT INTO metric_snapshots (
      company_id, metric_name, scope, scope_id,
      value, baseline_value, date, metadata
    )
    ...
    [
      companyId, metricName, sl, sid,
      metricValue, baseline ?? metricValue, forDate,
...
async function computeSlackMetrics(
  ...
  const impliedUnloggedActions = result.rows.filter(
    (row) => row.implied_action && !row.linked_jira_issue,
  ).length;
  ...
  return [
    {
      ...
      metadata: { thread_count: result.rows.length },
    },
    {
  ...
  return [
    {
      ...
      value: reopenRate,
      metadata: { reopened_count: reopenedCount, updated_count: updatedCount },
    },
    {
      metricName: 'jira.cycle_time_median',
      value: median(cycleTimeHours),
      metadata: { sampled_issues: cycleTimeHours.length, unit: 'hours' },
    },
  ];
}
...
async function computeGitHubMetrics(
  ...
  const reviewLatencies = lifecycleRows.rows
    .flatMap((row) => {
      if (!row.opened_at || !row.first_review_at) return [];
      const openedAt = row.opened_at as Date;
      if (openedAt < windowStart || openedAt > windowEnd) return [];
  ...
  return [
    {
      ...
      metadata: { sampled_prs: reviewLatencies.length, unit: 'hours' },
    },
    {
      metricName: 'github.pr_age_median',
      value: median(openPrAges),
      metadata: { sampled_open_prs: openPrAges.length, unit: 'hours' },
    },
  ];
}
...
export async function runDailyMetricsAggregation(companyId: string): Promise<void> {
  ...
  for (const team of teamsResult.rows) {
    await computeAndStoreMetrics(
      companyId, windowStart, windowEnd, snapshotDate,
      { teamId: team.id },
    );
  }
