...
/**
 * AI Orchestrator — Dual-provider AI diagnosis engine (OpenAI/Anthropic).
 *
 * Migrated from apps/worker/src/services/ai-orchestrator.ts.
 * Key changes:
 *   - Uses consumer config instead of worker config
 *   - Shared with on-diagnosis-requested handler
 *
 * All business logic preserved including fallback diagnosis,
 * JSON extraction, and dual-provider support.
 */
    ...
    fix_drafts: [
      {
        ...
        details: { source: 'deterministic-fallback' },
      },
    ],
  };
}
...
/**
 * Generate an AI diagnosis for a leak.
 * Falls back to deterministic diagnosis when LLM is unavailable.
 */
